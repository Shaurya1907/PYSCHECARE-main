...
python_chatbot_path = (
    base_dir / "Python-Mental-Health-Chatbot-main" / "Python-Mental-Health-Chatbot-main"
)

# Check if it actually exists before altering the environment
if python_chatbot_path.exists() and python_chatbot_path.is_dir():
    sys.path.insert(
        0, str(python_chatbot_path)
    )  # Using insert(0) is often safer to prioritize local modules
else:
...
def load_chatbot_model():
    ...
    try:
        ...
        intents_path = os.path.join(
            python_chatbot_path, "ChatbotWebsite", "static", "data", "intents.json"
        )

        # Check if we're working with the provided files or the ones in the original location
        if not os.path.exists(intents_path):
            intents_path = os.path.join(
                os.path.dirname(__file__), "chatbot_files", "intents.json"
            )
            model_path = os.path.join(
                os.path.dirname(__file__), "chatbot_files", "model.h5"
            )
            data_path = os.path.join(
                os.path.dirname(__file__), "chatbot_files", "data.json"
            )
